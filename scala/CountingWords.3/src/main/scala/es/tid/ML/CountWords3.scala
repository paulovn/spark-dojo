...
object CountWords {
  def main(args: Array[String]) {
    ...
    val counts = wordsKeyValue.reduceByKey( _ + _ ).collect

    //  And sort by frequency                                                                         
    val orderedCounts = counts.sortWith(_._2 > _._2)

    // Format output string 
    val formattedOutput = orderedCounts.map{ case (word, count) => f"[${word}] : ${count}" }

    // Save output to local file
    val outputFile = "quijote-words-3.txt"
    sc.parallelize(formattedOutput).saveAsTextFile(outputFile)
  }
}
